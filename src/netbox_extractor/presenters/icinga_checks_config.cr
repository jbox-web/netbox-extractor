  ...
  module Presenters
    module IcingaChecksConfig
      macro define_method_load_template_locals(service, key)
        ...
        private def load_template_locals_check_{{service.id}}
          if data = check_{{service.id}}?
            @template_locals.merge!({ "{{key.id}}" => data })
          end
        end
      end
      ...
      private def load_template_locals!
        load_template_locals_default
        load_template_locals_host

        load_template_locals_check_mysql
        load_template_locals_check_postgres
        load_template_locals_check_rabbit
        load_template_locals_check_php
        load_template_locals_check_nfs
        load_template_locals_check_netapp
        load_template_locals_check_drac
      end

      private def load_template_locals_default
        @template_locals["icinga_zone"] = @site.id
        @template_locals["host_zone"] = @site.id
        @template_locals["hostname"] = @host.name
        @template_locals["host_type"] = @host.netbox_host_type
        @template_locals["ipv4_address"] = @host.netbox_primary_ip
        @template_locals["check_type"] = icinga_check_type
        @template_locals["os_name"] = ansible_os_name
        @template_locals["os_family"] = @host.netbox_os_family
        @template_locals["os_distro"] = ansible_os_distro
        @template_locals["partitions_list"] = partitions_list
        @template_locals["tags"] = @host.netbox_tags
        @template_locals["load_warning"] = icinga_load_warning
        @template_locals["load_critical"] = icinga_load_critical
      end

      private def load_template_locals_host
        if @host.physical?
          data = {"vendor_name" => @host.netbox_vendor_name, "vendor_model" => @host.netbox_vendor_model}
          @template_locals.merge!(data)
        end

        if @host.virtual?
          data = {"hosting_node" => @host.hosting_node}
          @template_locals.merge!(data)
        end
      ...
      private def icinga_check_type
        ...
        if check
          check
        elsif @host.check_by_snmp?
          "snmp"
        elsif @host.check_by_ping?
          "ping"
        elsif @host.windows?
          "winrm"
        else
