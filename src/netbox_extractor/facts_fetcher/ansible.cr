  ...
  module FactsFetcher
    class Ansible
      ...
      def run
        ...
        WaitGroup.wait do |wg|
          @site.ansible.fetch_facts.inventories.each do |inventory_file|
            wg.spawn { fetch_facts(inventory_file) }
          end
        end
      end
      ...
      private def fetch_facts(filename)
        ...
        Log.info { "[#{@site.id}] Fetching Ansible facts for #{inventory_file}" }
      ...
      private def call_ansible(inventory)
        ...
        Log.debug { "[#{@site.id}] Running command: ansible-playbook #{args} #{env}" }
